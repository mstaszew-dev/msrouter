...
def _hit_to_text(h: dict, collection: str) -> str:
    ...
    formatters and the line-protocol transport so rendering stays in one place."""
    m = h.get("meta", {})
    if collection == "apps":
        return (
...
def _line_protocol_main(db_path: Path, campaign: Path) -> None:
    """Alternative to the MCP stdio main: read one JSON request per line, write
    one JSON response per line. Used by the Director's TypeScript RagClient
    (a thin line-protocol caller, not a full MCP client). Each request:
        {"id": <int>, "tool": "rag_search_apps"|"rag_search_docs", "args": {...}}
    Each response:
        {"id": <int>, "result": [{"score": float, "text": str}, ...]}
    or  {"id": <int>, "error": "<msg>"}
    """
    _log(f"line-protocol mode: db={db_path} campaign={campaign}")
    # Patch the module-level DB so _ensure_loaded picks the requested db.
    global DB
    DB = db_path
    req: dict = {}
    for raw_line in sys.stdin:
        ...
        try:
            ...
            k = int(args.get("k", 5))
            if not query.strip():
                sys.stdout.write(json.dumps({"id": rid, "error": "query is required"}) + "\n")
                sys.stdout.flush()
                continue
            if tool == "rag_search_apps":
                collection = "apps"
            elif tool == "rag_search_docs":
                collection = "docs"
            else:
                sys.stdout.write(json.dumps({"id": rid, "error": f"unknown tool {tool}"}) + "\n")
                sys.stdout.flush()
                continue
            hits = _search(query, collection, k)
            ...
            rid_out = req.get("id") if req else None
            sys.stdout.write(json.dumps({"id": rid_out, "error": str(e)}) + "\n")
            sys.stdout.flush()
