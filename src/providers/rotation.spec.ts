...
describe('CandidateQueue', () => {
  it('starts in the given order', () => {
    const q = new CandidateQueue(['a', 'b', 'c'], { log: silent });
  ...
  it('at() wraps around for out-of-range indices (round-robin)', () => {
    const q = new CandidateQueue(['a', 'b'], { log: silent });
  ...
  it('demote() of a middle item preserves relative order of others', () => {
    const q = new CandidateQueue(['a', 'b', 'c', 'd'], { log: silent });
    q.demote('b');
  ...
  it('demote() on a single-item queue is a no-op', () => {
    const q = new CandidateQueue(['only'], { log: silent });
    q.demote('only');
  ...
  it('demote() logs a warning with the label and item', () => {
    ...
    const q = new CandidateQueue(['a', 'b'], { log, label: 'opencode' });
    q.demote('a');
